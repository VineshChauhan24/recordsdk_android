...
import com.cgfay.cainfilter.multimedia.MediaMuxerWrapper;
import com.cgfay.cainfilter.multimedia.MediaVideoEncoder;
import com.cgfay.cainfilter.multimedia.QKMediaAudioEncoder;
...
public class QKEncoderManager {

    private static final String TAG = "QKEncoderManager";
    private static final boolean VERBOSE = false;

    private static QKEncoderManager mInstance;
    ...
    public static QKEncoderManager getInstance() {
        if (mInstance == null) {
            mInstance = new QKEncoderManager();
        }
        return mInstance;
    }

    private QKEncoderManager() {
...
//            Log.d(TAG, "the outpath is empty, auto-created path is : " + mRecorderOutputPath);
        ...
        try {
            ...
            new MediaVideoEncoder(mMuxerManager, listener, mVideoWidth, mVideoHeight,false);
            if (isEnableAudioRecording) {
                new QKMediaAudioEncoder(mMuxerManager, listener,false);
            }
//            Log.d(TAG,"initRecord6");
...
//        Log.d(TAG,"startRecording");
        if (mMuxerManager.getVideoEncoder() == null) {
            Log.e(TAG,"mMuxerManager.getVideoEncoder() == null");
            return;
        }
        // 释放之前的Egl
...
//        Log.d(TAG,"drawRecorderFrame1");
        if(mRecordWindowSurface == null){
            return;
        }
//        Log.d(TAG,"drawRecorderFrame2");

        if (mRecordWindowSurface != null) {
            mRecordWindowSurface.makeCurrent();
            drawRecordingFrame(currentTexture);
//            Log.d(TAG,"drawRecorderFrame3");
...
//        Log.e(TAG,"drawAudioData");
        if(mMuxerManager == null){
            return;
        }
        if (mMuxerManager.getAudioEncoder() == null) {
            Log.e(TAG,"mMuxerManager.getAudioEncoder() == null");
            return;
        }
        QKMediaAudioEncoder audioEncoder = (QKMediaAudioEncoder)mMuxerManager.getAudioEncoder();
        audioEncoder.addNewAudioFrame(data.getData(),data.getPts());
//        Log.e(TAG,"finished");
