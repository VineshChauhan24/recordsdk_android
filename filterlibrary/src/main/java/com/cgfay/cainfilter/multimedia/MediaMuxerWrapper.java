...
public class MediaMuxerWrapper {
    ...
    public void stopRecording() {
        if (mVideoEncoder != null){
            mVideoEncoder.retainMuxer();
            final MediaEncoder useVideoEncoder = mVideoEncoder;
            new Thread(new Runnable() {
                @Override
                public void run() {
                    useVideoEncoder.stopRecording();
                }
            }).start();
        }
		mVideoEncoder = null;
        if (mAudioEncoder != null){
            mAudioEncoder.retainMuxer();
            final MediaEncoder useAudioEncoder = mAudioEncoder;
            new Thread(new Runnable() {
                @Override
                public void run() {
                    useAudioEncoder.stopRecording();
                }
            }).start();
        }
        mAudioEncoder = null;
...
//**********************************************************************
    ...
    /*package*/ void addEncoder(final MediaEncoder encoder) {
        ...
        }  else if (encoder instanceof QKMediaAudioEncoder) {
            if (mAudioEncoder != null)
                throw new IllegalArgumentException("Audio encoder already added.");
            mAudioEncoder = encoder;
        } else
