...
import com.cgfay.cainfilter.utils.CameraInfo;
...
//        FaceTrackManager.getInstance().onFaceTracking(data);
    ...
    void startRecording() {
        if (mEglCore != null && isPreviewing) {
            // 设置渲染Texture 的宽高
            RecordManager.getInstance().setTextureSize(mViewWidth, mViewHeight);//mImageWidth mImageHeight
            // 设置预览大小
